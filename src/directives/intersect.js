export function createIntersection(insideTime = 1, outsideTime = 1) {
  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      ...
      const { inside, outside } = cache.get(target)

      // observation target is inside viewport
      if (entry.intersectionRatio > 0) {
        if (inside && typeof inside === 'function') {
          if (target[INSIDE_COUNTE] > 0) {
            inside(target)
            target[INSIDE_COUNTE]--
          }
        }
      } else {
        if (outside && typeof outside === 'function') {
          if (target[OUTSIDE_COUNTE] > 0) {
            outside(target)
            target[OUTSIDE_COUNTE]--
          }
        }
      }

      if (insideTime === 0 && outsideTime === 0) {
        io.unobserve(target)
      }
    })
  })
